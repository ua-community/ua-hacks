...
class ControlledFrameController {
  ...
  #addControlledFrameAttributeHandlers() {
    ...
    $('#fullscreen_btn').addEventListener('click', this.#setFullscreen.bind(this));
  }
  ...
  #setFullscreen(e) {
    const allowed = $('#fullscreen_chk').checked;
    this.controlledFrame.allow = allowed ? 'fullscreen' : '';
    Log.info(`Fullscreen ${allowed ? 'enabled' : 'disabled'}`);
  }
  ...
  #onpermissionrequest(e) {
    ...
    if (e.permission === 'fullscreen') {
      // Only allow fullscreen if the checkbox is checked
      if ($('#fullscreen_chk').checked) {
        Log.info('Fullscreen permission request granted');
        e.request.allow();
      } else {
        Log.info('Fullscreen permission request denied');
        e.request.deny();
      }
      return;
    }

    // Handle other permission types as before
    e.request.allow();
  }
